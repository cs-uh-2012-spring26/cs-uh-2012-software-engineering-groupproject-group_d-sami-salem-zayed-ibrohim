...
### 1. Implementation of the Strategy Pattern (Features 2 & 5)
In Sprint 2, notification delivery was tightly coupled with business logic. In Sprint 3, we introduced a formal **Strategy Pattern**.
* **Structural Change:** Added `NotificationDispatcher` and the `«interface» NotificationService`.
* **Impact:** The system is now **Cloud Agnostic**. We can swap between `SESEmailService` (AWS) and `TelegramNotificationService` without modifying the `ReminderService`. This satisfies the **Open/Closed Principle**.

### 2. Advanced Domain Modeling & DTOs (Feature 6)
Sprint 3 introduces complex recurrence logic that was entirely absent or hard-coded in the previous sprint.
* **Structural Change:** Added `CreateClassRequest`, `RecurrenceRule`, and `ClassSchedule`.
* **Impact:** By moving calendar math and validation into dedicated **Data Transfer Objects (DTOs)**, we have achieved **High Cohesion**. The `ClassService` no longer calculates dates; it simply orchestrates the persistence of records generated by the DTO.

### 3. Decoupled Identity Context
Security logic in Sprint 2 relied on raw dictionary access from JWTs. Sprint 3 formalizes this through an **Identity Context**.
* **Structural Change:** Introduced the `AuthenticatedUser` frozen dataclass.
* **Impact:** Every API Resource now depends on this immutable object. This centralizes **Role-Based Access Control (RBAC)**, ensuring that identity validation is consistent and decoupled from the `flask-jwt-extended` framework.

### 4. Robust Service Orchestration (The Facade Pattern)
The Service Layer has evolved from simple wrappers to powerful **Orchestrators**.
* **Structural Change:** `ReminderService` and `BookingService` now coordinate between three distinct resources (User, Class, and Booking).
* **Impact:** This prevents **Resource Bloat**. Database logic remains pure within the Resources, while complex workflows—such as checking trainer overlaps or building multi-channel reminder messages—are handled by the Service layer.

---

## Architectural Summary
> "The Sprint 3 architecture successfully achieves **Low Coupling** by separating infrastructure concerns (AWS, Telegram) from core gym business logic. Through the use of **Interfaces** and **Value Objects**, the system is now prepared for future scaling, such as adding new notification channels or complex membership tiers, with minimal code changes."
