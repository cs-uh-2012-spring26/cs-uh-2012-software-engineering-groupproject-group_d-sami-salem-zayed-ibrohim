...
### 3.1 Delegation of API Tasks
In Sprint 3A, the APIs were directly managing resources. In Sprint 3B, we redistributed these tasks across various middle classes.

**What Changed?** Instead of the APIs authenticating directly with the user resource or parsing JWT tokens themselves, they now simply call a method from the new 'AuthenticatedUser' class.<br>
**Impact:** This removes the need for the APIs to be informed of the database or authentication implementation details. They simply operate by calling high-level functions.

### 3.2 Architectural Layering (Introduction of Service Classes)
In Sprint 3A, the APIs had direct access to manage database resources. Now, they must go through a dedicated Service Layer.

**What Changed?** Several dedicated service classes, including 'AuthService', 'BookingService', and 'ClassService', were added to act as a buffer between the web layer and the data layer.<br>
**Impact:** These services now act as the managers for the resources, keeping the data layer safe.

### 3.3 Pluggable Notification System (Strategy Pattern)
In Sprint 3A, the Notification Service only handled Email services. Now, the system can handle both Email and Telegram services.

**What Changed?** A Strategy Pattern was implemented by introducing a 'NotificationService' interface and a 'NotificationDispatcher'.<br>
**Impact:** The system now follows the Open/Closed Principle. We can plug in additional services (like SMS) in the future by adding a new strategy class without changing any of the core reminder logic.

### 3.4 Use of Private Methods
Sprint 3A suffered from "Long Method" code smells, where single public functions reached 75 lines because they lacked internal delegation. All class methods being public also exposed internal helper logic to the entire system. 

**What Changed?** Large blocks of code were refactored into smaller, private helper functions (marked with the - prefix in the UML diagram).<br>
**Impact:** This improved Encapsulation. By hiding "implementation details" inside private methods, we ensure that external classes can only interact with the intended public interface, making the code easier to read and debug.

### 3.5 High Cohesion through Domain Modeling (DTOs)
In Sprint 3A, class data was handled as "Primitives" (raw strings/numbers). Sprint 3B introduces Advanced Domain Modeling to handle recurring class logic.

**What Changed?** Data Transfer Objects (DTOs) and value objects like 'CreateClassRequest', 'RecurrenceRule', and 'ClassSchedule' were introduced.<br>
**Impact:** High Cohesion was achieved. Instead of the Service handling calendar math, specialized objects encapsulate that operation. This makes the system significantly more maintainable.

### 3.6 Reduction of Parameter Complexity
In Sprint 3A, database methods suffered from "Long Parameter Lists," requiring long sequences of individual inputs like title, trainer_id, and dates.

**What Changed?** Methods were refactored to accept Dictionaries or DTOs instead of long sequences of primitive arguments.<br>
**Impact:** This reduced "Parameter Obsession." By grouping related data into a single object, the method signatures became cleaner and the system became more flexible. New class attributes can be added without breaking existing function calls.
